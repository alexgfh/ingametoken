...
function serve() {
	if (https_server) {
		...
		try { server.listen(port, () => { console.log(ip, ' server %s listening at %s', hostname, server.url); }); }
		catch (e) {
			console.log(e);
			return false;
		}
...
function BurnToken(req, res, next) {
	...
	return burnToken(req.params)
		...
		.catch(function (e) { console.log("burntoken failed:", e); res.send(500, e); return next() })
}
...
function CheckTransfer(req, res, next) {
	...
	return checkTransfer(req.params)
		...
		.catch(function (e) { console.log("checktransfer failed:", e); res.send(500, e); return next() })
}
...
function CoinInfo(req, res, next) {
	...
	return coinInfo(req.params)
		...
		.catch(function (e) { console.log("coininfo failed:", e); res.send(500, e); return next() })
}
...
function CreateToken(req, res, next) {
	...
	return createToken(req.params)
		...
		.catch(function (e) { console.log("createtoken failed:", e); res.send(500, e); return next() })
}
...
function MintToken(req, res, next) {
	...
	return mintToken(req.params)
		...
		.catch(function (e) { console.log("minttoken failed:", e); res.send(500, e); return next() })
}
...
function UserBalance(req, res, next) {
	...
	return userBalance(req.params)
		...
		.catch(function (e) { console.log("userbalance failed:", e); res.send(500, e); return next() })
}
////////////
...
async function createToken(params) {
	...
	let to = await findAssociatedTokenAccountPublicKey(owner.publicKey, mintAccount.publicKey);
	let id = randomId();
	...
	let lamports = await connection.getMinimumBalanceForRentExemption(82, "singleGossip");
	...
	let amount64 = new Numberu64(params.supply * Math.pow(10, params.decimals)).toBuffer();
	cIx = createIx(
		feePayer.publicKey,
		to,
		owner.publicKey,
	...
	//
	let createMintTx = new Transaction().add(createMint).add(initMintIx).add(cIx).add(minttoix).add(fundtx).add(fixSupply);
...
async function getTokenBalance(connection, publicKey) {
	...
	let balance = 0;
	if (info) { balance = get64Value(info.data.slice(64, 72).reverse()); }
	return balance;
}
...
async function mintToken(params) {
	...
	let tokenInfo = Settings.Accounts[params.hash];
	if (params.to === tokenInfo.TokenAddress || tokenInfo.TokenAddress === params.solAddress) {
	...
	if (params.to) {
		...
		to = await findAssociatedTokenAccountPublicKey(new PublicKey(params.solAddress), new PublicKey(tokenInfo.Mint));
	}
	let OldBalance = 0;
...
async function userBalance(params) {
	...
	let Balance = 0;
	if (accountInfo) { Balance = await getTokenBalance(connection, pk); }
